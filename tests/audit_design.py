...
def check(name, css, html):
    ...
    # A bare [hidden] loses to any author `display` rule, because author styles
    # outrank the UA sheet. If the markup uses the attribute, the reset must
    # exist. (jsdom's cascade does not reproduce this, so check it statically.)
    uses_hidden = re.search(r"<[^>]+\shidden(\s|>|=)", html) is not None
    has_reset = re.search(r"\[hidden\]\s*\{[^}]*display\s*:\s*none\s*!important",
                          css) is not None
    ok = not uses_hidden or has_reset
    note(ok, "the [hidden] attribute is backed by an !important reset",
         "" if ok else "markup uses hidden= but there is no [hidden] reset")
